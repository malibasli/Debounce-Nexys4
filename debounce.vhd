...
if (rising_edge(clk)) then
	...
	case state is
		...
		when S_ZERO =>
			...
			if (signal_i = '1') then  -- BURAI BEKLEME ALANI GÝRÝÞ 1 OLUNCA zerotoone state geç 
				state	<= S_ZEROTOONE;
			end if;
		
		when S_ZEROTOONE =>
		
			signal_o	<= '0'; -- çýkþý sýfýlra
			timer_en	<= '1'; -- süreyi baþlat 
			
			if (timer_tick = '1') then  -- bayrak kalkarsa yani süre boyunca 1 de kalýrsa signal_i
				state		<= S_ONE;
				timer_en	<= '0';
			end if;
		...
		when S_ONE =>
			...
			signal_o	<= '1'; -- süre boyunca 1 de kalýnca çýkkýþý 1 e çektik 
		
			if (signal_i = '0') then      --
				state	<= S_ONETOZERO;   -- 0 dan 1 e geçince eðer input 0 olursa onetozero ya gidiyor
			end if;		                  --
